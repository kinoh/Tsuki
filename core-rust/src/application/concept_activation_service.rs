    ...
    G: ConceptGraphActivationReader + ConceptGraphOps + ?Sized,
{
    ...
    if !seeds.is_empty() {
        if let Err(err) = graph.recall_query(seeds.to_vec(), RECALL_MAX_HOP).await {
...
mod tests {
    ...
    async fn renders_active_nodes_as_text() {
        ...
        let result = activate_concepts(&["好奇心".to_string()], 8, &graph).await;
    ...
    async fn returns_none_when_no_active_nodes() {
        ...
        let result = activate_concepts(&[], 8, &graph).await;
    ...
    async fn records_recall_error_but_still_returns_active_nodes() {
        ...
        let result = activate_concepts(&["seed".to_string()], 8, &graph).await;
