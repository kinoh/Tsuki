...
impl StateStore for DbStateStore {
    fn set(
        ...
        let related_keys_json =
            serde_json::to_string(&related_keys).unwrap_or_else(|_| "[]".to_string());
        let metadata_json = serde_json::to_string(&metadata).unwrap_or_else(|_| "{}".to_string());
        let db = self.db.clone();
        let updated_at = block_in_place(|| {
            Handle::current().block_on(async {
                db.upsert_state_record(&key, &content, &related_keys_json, &metadata_json)
                    .await
            })
        })
    ...
    fn get(&self, key: &str) -> Option<StateRecord> {
        let db = self.db.clone();
        let result =
            block_in_place(|| Handle::current().block_on(async { db.get_state_record(key).await }));
        match result {
    ...
    fn search(&self, query: &str, limit: usize) -> Vec<StateRecord> {
        ...
        match result {
            Ok(rows) => rows
                .into_iter()
                .map(
                    |(key, content, related_keys_json, metadata_json, updated_at)| {
                        let related_keys =
                            serde_json::from_str(&related_keys_json).unwrap_or_default();
                        let metadata =
