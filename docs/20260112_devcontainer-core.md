...
- Disable the base `core` build in the devcontainer override with `!reset` to avoid Alpine-based images that cannot run host-built glibc binaries.
...
- `.devcontainer/compose.yaml` overrides `core` with:
  ...
  - `build: !reset null` to prevent the Alpine-based image from `compose.yaml` being used.
