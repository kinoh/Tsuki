...
- Install `pnpm` in the user prefix (`$HOME/.npm-global`) via `postCreateCommand` to avoid image rebuilds and avoid version pinning.
...
- `.devcontainer/devcontainer.json` installs `pnpm` on container creation via `postCreateCommand` and extends `PATH` with `$HOME/.npm-global/bin`.
